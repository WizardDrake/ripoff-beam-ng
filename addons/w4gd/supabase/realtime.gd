...
## If [code]true[/code] the client will retry connecting after a disconnection
## or connection error.
var reconnect_enabled := true
## The maximum number of times the client will retry connecting to the remote
## server upon disconnection. See [member reconnect_enabled].
var reconnect_max_retries := 3
## The base backoff time before trying reconnecting. See [member reconnect_enabled].
## [br]
## [b]Note[/b]: The final backoff time will be [code]reconnect_backoff * pow(2, retry)[/code].
var reconnect_backoff := 0.8
...
var _json := JSON.new()
var _topics := {}
var _poll_timer := SafeTimer.create_interval(0.001).connected_to(poll) # won't be less then process time in any case.
var _reconnect_timer := SafeTimer.create_timeout().connected_to(_try_connect)
var _closing := false
var _responses := {}
var _reconnect_retry := 0
...
func _init(p_node: Node, p_url: String, p_identity, p_tls_options: TLSOptions = null):
	...
	_identity.identity_changed.connect(_reset_topic_states)


## Connect to Supabase realtime via WebSockets.
func connect_socket() -> Error:
	_closing = false
	_reconnect_retry = 0
...
func _try_connect() -> Error:
	if _endpoint == "":
		return ERR_UNCONFIGURED
	var url = "%s?apikey=%s&vsn=1.0.0" % [
		_endpoint,
		_identity.get_api_key()
	]
	return _ws.connect_to_url(url, _tls_options)
...
func should_retry_connection() -> bool:
	return reconnect_enabled and _reconnect_retry < reconnect_max_retries
...
func _connection_closed(p_was_clean:=false):
	...
	if not _closing and should_retry_connection():
		_reconnect_timer.start(reconnect_backoff * pow(2, _reconnect_retry))
		_reconnect_retry += 1
