...
const AUTO_CLEAN_UP_SETTING = 'w4games/online/analytics_cleanup_on_quit'
...
var _send_timer := SafeTimer.create_interval(60.0, false).connected_to(_on_send_timer_timeout)
var _send_queue := {}
var _last_flush := 0
var _sending := false
var _started := false
...
func send_event(p_event_name: String, p_props := {}, user_id := '') -> void:
	...
	var timestamp := W4Utils.get_current_time()
...
func flush() -> void:
	...
	if _session_id == null:
		...
		var result = await _client.rest.rpc('w4public.analytics_session_start', data).async()
		if result.is_error():
	...
	var result = await _client.rest.rpc('w4public.analytics_events_create', data).async()
	if result.is_error():
		error.emit(result.message)
		return
