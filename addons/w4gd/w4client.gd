...
const Party = preload("matchmaker/party.gd")
const Analytics = preload("analytics/analytics.gd")
const IDP = preload("idp/idp.gd")
...
# Quick aliases.
var auth: IDP:
	get: return client.auth as IDP if client != null else null
var rest: Rest:
...
func _init(config: Dictionary = {}, service:=false):
	...
	client.auth = IDP.new(client.get_rest_client(), client.get_identity())
...
##
## Can be configured in the project settings ([code]w4games/servers/default_port[/code]), and
