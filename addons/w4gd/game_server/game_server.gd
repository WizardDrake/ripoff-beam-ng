...
var _health_timer := SafeTimer.create_interval(_health_check_interval, false).connected_to(_on_health_timer_timeout)
var _match_ready_timer := SafeTimer.create_timeout(_player_join_timeout, false).connected_to(_on_match_ready_timer_timeout)
...
func _init():
	var project_settings := {
		health_check_interval = 'w4games/servers/health_check_interval',
		player_join_timeout = 'w4games/servers/player_join_timeout',
		minimum_players = 'w4games/servers/minimum_players',
		auto_shutdown_on_match_failure = 'w4games/servers/auto_shutdown_on_match_failure',
	}
...
func _process_player_registration(p_peer_id: int, p_player: PlayerRegistration) -> void:
	...
	hmac.start(HashingContext.HASH_SHA256, _lobby_password.to_utf8_buffer())
	hmac.update((p_player.player_id + ':' + _lobby_id).to_utf8_buffer())
	var correct_password := hmac.finish().hex_encode()
