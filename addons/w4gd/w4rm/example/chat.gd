...
class Profile:
	...
	static func _w4rm_type_options(opts) -> void:
		opts["id"] = W4RM.tref("auth.users", {
			default = "auth.uid()",
			external = true,
			on_delete = W4RM.W4RMTables.W4RMReferentialAction.CASCADE,
		})
...
class ChatMessage:
	...
	static func _w4rm_type_options(opts) -> void:
		opts['lobby_id'] = W4RM.tref("w4lobby.lobby", {
			external = true,
			on_delete = W4RM.W4RMTables.W4RMReferentialAction.CASCADE,
		})
		opts['profile_id'] = W4RM.tref('Profile', {
			default = 'auth.uid()',
			on_delete = W4RM.W4RMTables.W4RMReferentialAction.SET_NULL,
			nullable = true,
		})

	static func _w4rm_security_policies(policies) -> void:
		policies["Authenticated users can view messages in lobbies they are in"] = \
			W4RM.build_security_policy_for('authenticated').can_select_if('w4lobby.user_in_lobby(lobby_id)');
		policies["Authenticated users can only post messages in lobbies they are in"] = \
			W4RM.build_security_policy_for('authenticated').can_insert_if('w4lobby.user_in_lobby(lobby_id)');

	static func _w4rm_triggers(triggers, builder) -> void:
		builder.force_default('id')
		builder.force_default_on_insert('profile_id')
