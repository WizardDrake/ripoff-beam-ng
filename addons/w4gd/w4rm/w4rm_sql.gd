...
static func sql_referential_action(p_action: Tables.W4RMReferentialAction) -> String:
	match p_action:
		Tables.W4RMReferentialAction.NO_ACTION:
			return "NO ACTION"
		Tables.W4RMReferentialAction.CASCADE:
			return "CASCADE"
		Tables.W4RMReferentialAction.SET_NULL:
			return "SET NULL"
		Tables.W4RMReferentialAction.SET_DEFAULT:
			return "SET DEFAULT"
		Tables.W4RMReferentialAction.RESTRICT:
			return "RESTRICT"
		_:
			push_error("Invalid p_action parameter: %s" % p_action)
			return "NO ACTION"
...
static func get_create_foreign_key(p_source_table: String, p_source_column: String,
		p_schema: String, p_table: String, p_column: String, p_on_delete: Tables.W4RMReferentialAction,
		p_on_update: Tables.W4RMReferentialAction) -> String:
	...
	""" % [
		...
		sql_referential_action(p_on_delete),
		sql_referential_action(p_on_update)
	]
...
static func get_create_table_foreign_keys(p_table) -> Array:
	...
	for k in opts:
		if not (opts[k] is Tables.W4RMTable.TableReference):
			continue
		var opt := opts[k] as Tables.W4RMTable.TableReference
		if opt.on_delete == Tables.W4RMReferentialAction.SET_NULL:	
			assert(opt.nullable, "%s: can't set SET NULL on delete if column '%s' is not nullable." % [p_table.get_table_name(), k])
		out.append(get_create_foreign_key(p_table.get_table_name(), k, opt.schema, opt.table, opt.id, opt.on_delete, opt.on_update))
	return out
