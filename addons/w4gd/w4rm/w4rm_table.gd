...
class W4RMTable:
	...
	class TableReference extends MemberOption:
		...
		var target := &""
		var external := false
		var on_delete := W4RMReferentialAction.NO_ACTION
		var on_update := W4RMReferentialAction.NO_ACTION
...
static func w4rm_reference(opts:={}):
	...
	tr.on_delete = opts.get("on_delete", W4RMReferentialAction.NO_ACTION)
	tr.on_update = opts.get("on_update", W4RMReferentialAction.NO_ACTION)
	return tr
