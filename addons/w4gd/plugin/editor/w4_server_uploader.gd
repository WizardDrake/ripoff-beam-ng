...
func _update_fleet_options() -> void:
	...
	## TODO FIXME!
	var result = await sdk.client.rest.rpc("w4service.fleet_get_all").async()
	if result.is_error():
...
func _do_upload_and_create_build(p_build_name: String, p_fleet: Dictionary) -> bool:
	...
	result = await sdk.client.rest.rpc("w4service.gameserver_build_create", data).async()
	if result.is_error():
	...
	if not p_fleet.is_empty():
		...
		result = await sdk.client.rest.rpc("w4service.fleet_update", fleet).async()
		if result.is_error():
