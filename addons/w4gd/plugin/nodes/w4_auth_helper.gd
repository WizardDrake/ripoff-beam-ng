...
## Emitted when the current request retries.
signal retrying()
...
func login_auto() -> void:
	...
	if Engine.has_singleton("Steam"):
		...
		var inited : Dictionary = steam.steamInitEx(0, false)
		if inited.has("status") and inited["status"] == 0:
			if not get_tree().process_frame.is_connected(steam.run_callbacks):
				get_tree().process_frame.connect(steam.run_callbacks)
...
func login_steam(identity: String, ticket_buffer: String) -> void:
	...
	_request.retrying.connect(_on_request_retrying)
	_request.async()
...
# For compatibility with Godot < 4.3 that doesn't support treating emit like a Callable
func _on_request_retrying():
	retrying.emit()
