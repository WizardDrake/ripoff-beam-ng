...
enum IdentityMode {DISABLED = 0, DIRECT = 1, OAUTH = 2, SAML = 3}
...
func _idp_login(data):
	var mode := IdentityMode.DIRECT
	if ProjectSettings.has_setting("w4games/online/identity_mode") and \
		typeof(ProjectSettings.get_setting("w4games/online/identity_mode")) == TYPE_INT:
		mode = ProjectSettings.get_setting("w4games/online/identity_mode")
	match mode:
		IdentityMode.DIRECT:
			return _idp_login_direct(data)
		IdentityMode.SAML:
			return _idp_login_sso(data)
		IdentityMode.DISABLED:
			push_error("Identtiy provider is disabled")
			return null
		IdentityMode.OAUTH:
			push_error("Identity provider OAuth mode is not yet supported")
			return null
		_:
			push_error("Invalid identity provider mode")
			return null


func _idp_login_direct(data):
	return idp.POST(
		"/login", data
	).then(func(result):
