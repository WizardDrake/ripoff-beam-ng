...
func initialize():
	if not Vector3.ONE.is_equal_approx(scale):
		var s := scale.x
			...
			if child is CollisionShape3D:
				...
				if child.shape:
					...
					if new_shape is BoxShape3D:
						new_shape.size *= s
					elif new_shape is SphereShape3D:
						new_shape.radius *= s
		...
		front_tire_radius *= s
		rear_tire_radius *= s
		front_spring_length *= s
		rear_spring_length *= s
		vehicle_mass *= (s * s)
		scale = Vector3.ONE
	...
	mass = vehicle_mass
	# Reduces tunneling through thin floors / fast impacts (helps wheel-body vs ground).
	continuous_cd = true
