...
## Positive pushes the wheel mesh up (helps if the tire mesh is larger than [tire_radius] or to avoid floor clipping).
@export var wheel_visual_lift_offset: float = 0.0

var wheel_mass := 15.0
var tire_radius := 0.155
...
func _physics_process(delta: float) -> void:
	# Sync with physics step (same frame as process_forces) so visuals don't lag behind the body and clip through the floor.
	if wheel_node:
		wheel_node.position.y = minf(0.0, -spring_current_length) + wheel_visual_lift_offset
