...
func _add_collision_to_building(node: Node):
	...
	sb.name = "SolidPhysics"
	node.add_child(sb)
	# Owner will be set correctly by generate_map.gd recursively!
	
	_add_shapes_recursive(node, node, sb)

func _add_shapes_recursive(root: Node, current: Node, sb: StaticBody3D):
	if current is MeshInstance3D and current.mesh:
		var cs = CollisionShape3D.new()
		# Use actual mesh geometry for high-accuracy "trimesh" collision
		var shape = current.mesh.create_trimesh_shape()
		cs.shape = shape
		
		# Root-relative transform to place the shape correctly in the StaticBody
		# We must NOT use global_transform here because it may be stale or incorrect
		# during procedural generation when nodes are not yet in the full scene tree.
		cs.transform = _get_relative_transform(current, root)
		
		sb.add_child(cs)
		
	for child in current.get_children():
		if child != sb:
			_add_shapes_recursive(root, child, sb)

func _get_relative_transform(child: Node3D, ancestor: Node):
